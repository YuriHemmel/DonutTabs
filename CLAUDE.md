...
## Looking ahead to Plano 13 and beyond

Plano 12 is done: comandos `export_config` / `import_config` + helpers `do_export` / `do_import` (`pub(crate)`, unit-tested sem runtime Tauri) entregam backup/restore do `config.json` para um caminho arbitrário. Export reusa `save_atomic` no path do user; import reusa `load_from_path` (que já valida + migra v1→v2), persiste no `state.config_path` canônico (não no source — drive removível seria perda silenciosa), reconcilia o atalho global do novo perfil ativo via `shortcut::set_from_config` (best-effort: colisão loga e mantém o atalho anterior), troca o estado em memória e emite `CONFIG_CHANGED_EVENT`. Capability `dialog:allow-save` desbloqueia o picker. UI: dois botões no fieldset "Sistema" do `<AppearanceSection>` com `window.confirm` antes do import.

Próximo (Plano 13): `kind: "app"` + `kind: "script"` em uma slice combinada com `tauri-plugin-shell` (compartilham mecânica de spawn + UX de segurança). `app` = friendly name resolution (PATH/Windows registry App Paths/macOS .app); `script` = shell command com modal de confirmação dedicado.

Plano 14+: temas customizáveis, busca rápida por aba, sub-donuts, auto-updater Tauri.
