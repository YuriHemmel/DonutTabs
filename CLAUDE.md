...
## Looking ahead to Plano 10 and beyond

Plano 9 is done: right-click context menu (`<SliceContextMenu>` com Abrir tudo / Editar / Excluir, ESC + click-outside fecham, hover-hold é desabilitado enquanto o menu está aberto), favicons automáticos com cache em disco (`<app_config>/favicons/<sha256(origin)>.bin`, TTL 7d, comando `fetch_favicon` + hook `useFavicon`) e suporte a ícones Lucide via prefixo de string `lucide:Name` (decisão de render via `<IconRenderer>`; `<IconPicker>` modal com tabs Emoji + Lucide buscável atende ao `<TabEditor>` e `<ProfileEditor>`). O schema permanece `icon: string?` — o prefixo `lucide:` é convenção do frontend, não tem impacto em validação. `stripLetters` e o cap de 1 grafema do input de ícone são pulados quando o valor começa com `lucide:` para que tokens como `lucide:CoffeeIcon` sobrevivam ao input controlado.
