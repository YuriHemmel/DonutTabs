...
## Looking ahead to Plano 12 and beyond

Plano 11 is done: cada variante de `Item` ganhou `open_with: Option<String>` (`#[serde(default, skip_serializing_if)]` mantém configs antigas compatíveis); `Opener` trait + `TauriOpener` propagam `with: Option<&str>` ao `tauri-plugin-opener`; `validate.rs` rejeita `Some("")`/whitespace via `open_with_empty` (`None` é o canônico "use default"); `<ItemListEditor>` ganhou 4ª coluna "Abrir com" e `<TabEditor>` mapeia `ItemDraft.openWith` ↔ `Item.openWith` com trim e null-collapse no submit. `Tab.openMode` continua vestigial (campo armazenado/validado/editado mas **não lido pelo launcher**) — pode ressuscitar quando o plugin ganhar controle real de tab/window por browser.

Próximo (Plano 12): `kind: "app"` (resolução PATH/registry para nomes amigáveis tipo "Edge", "Chrome"), com decisão entre reusar `open_path` (input absoluto) ou adicionar resolver dedicado.

Plano 13: `kind: "script"` (shell command com modal de confirmação de segurança e flag de profile gating).
