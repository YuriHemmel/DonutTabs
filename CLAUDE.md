...
- `src/donut/{Slice,CenterCircle,PaginationDots,HoverHoldOverlay,Donut}.tsx` — presentational SVG components. The Donut renders a single page at a time, navigates via wheel (respecting `wheelDirection`) and clickable indicator dots. The ⚙ on `CenterCircle`'s left half is clickable (opens Settings); the right half still shows the profile glyph 👤 reserved for the profile switcher (Plano 6). HoverHoldOverlay paints the radial fill while holding and the ✏️/🗑️ split + Sim/Não confirm in the action phases.
...
- `src/entry/donut.tsx` — window entrypoint: bootstraps i18next from config before React mounts, wires ESC / click-outside / window-blur → `hideDonut`, delegates selection to `openTab`, listens for `config-changed` to refresh tabs AND call `changeLanguage`, surfaces launch failures as a dismissible localized toast. Routes gear clicks to `openSettings("new-tab"?)`; routes hover-hold edit to `openSettings("edit-tab:<id>")` + `hideDonut`; routes hover-hold delete to `ipc.deleteTab` (donut keeps showing — `config-changed` removes the slice).
- `src/entry/settings.tsx` + `src/settings/*` — the Settings window. `SettingsApp` routes between three sections via `<SectionTabs>` (`Abas` | `Aparência` | `Atalho`). `useConfig` is the single source of truth for config: loads on mount via `get_config` and subscribes to `config-changed`, with `saveTab`/`deleteTab`/`setShortcut`/`setTheme`/`setLanguage` helpers that round-trip through IPC. `applyIntent` parses both `"new-tab"` and `"edit-tab:<id>"` (validating the id against the live config; intents that arrive before the config snapshot are buffered and replayed). `TabEditor` validates name-or-icon / at-least-one-URL / `new URL(...)` client-side. `AppearanceSection` hosts theme radio + language select. `ShortcutSection` wraps `<ShortcutRecorder>`, which uses the pure `buildCombo(e)` helper to assemble the Tauri shortcut string from a keydown, rejecting reserved keys and modifier-only combos. Server-side `AppError`s render via `translateAppError`.
...
## Looking ahead to Plano 6 and beyond

Plano 5 is done: donut gestures — pagination via wheel + clickable indicator dots, hover-hold → ✏️/🗑️ split → confirm Sim/Não inline. `edit-tab:<id>` intent piggybacks on the existing intent infra and routes to the matching tab in the Settings.

Next slices (in order):

1. **Plano 6 — Perfis**: schema v2 + migração v1→v2 + profile switcher no lado direito do centro (atalho e tema por perfil).
2. **Plano 7 — Polimento**: menu de contexto nas fatias, favicons / Lucide, drag-and-drop para reordenar, autostart. (O donut permanece propositalmente em paleta escura — é overlay transparente; alternar para tema claro brigaria com o fundo da área de trabalho.)
