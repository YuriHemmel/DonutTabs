...
## Looking ahead to Plano 9 and beyond

Plano 8 is done: `useDragReorder` (HTML5 native DnD, sem dependência nova) + comandos `reorder_tabs` / `reorder_profiles` permitem reordenar abas dentro de um perfil e perfis no `<DraggableProfileList>` (que substituiu o `<select>` do `<ProfilePicker>`). A nova ordem é a ordem canônica do `Vec` no JSON; o donut consome `tabs` / `profiles` na mesma ordem.

Next slices (in order):

1. **Plano 9 — Menu de contexto + favicons** ([docs/plans/09-menu-contexto-favicons.md](docs/plans/09-menu-contexto-favicons.md)): right-click nas fatias e ícones automáticos a partir das URLs.
